...
class CreditRepository(application: Application) {
    private val context: Context
    private val profileDao: ProfileDao
    private val db: PharmacyDatabase
    val creditObservable = MutableLiveData<Resource<CreditReportData>>()
    init {
    ...
    private fun execute() {
        GlobalScope.launch(context = Dispatchers.Main) {
            val call = RequestService.getService(getToken()).creditReport()
            call.enqueue(object : Callback<CreditReportData> {
                override fun onFailure(call: Call<CreditReportData>?, t: Throwable?) {
                    setIsError(t.toString())
                }

                override fun onResponse(call: Call<CreditReportData>?, response: Response<CreditReportData>?) {
                    if (response != null) {
    ...
    private fun setIsLoading() {
        creditObservable.postValue(Resource.loading(null))
    }

    private fun setIsSuccessful(parameters: CreditReportData) {
        creditObservable.postValue(Resource.success(parameters))
    }

    private fun setIsError(message: String) {
        creditObservable.postValue(Resource.error(message, null))
    }
    fun getToken(): String {
