...
class SupplierRepository (application: Application) {
    private val context: Context
    private val profileDao: ProfileDao
    private val db: PharmacyDatabase
    val supplierObservable = MutableLiveData<Resource<SupplierData>>()
    init {
    ...
    private fun execute() {
        GlobalScope.launch(context = Dispatchers.Main) {
            val call = RequestService.getService(getToken()).suppliers()
    ...
    private fun active() {
        GlobalScope.launch(context = Dispatchers.Main) {
            val call = RequestService.getService(getToken()).activeSuppliers()
            call.enqueue(object : Callback<SupplierData> {
                override fun onFailure(call: Call<SupplierData>?, t: Throwable?) {
                    setIsError(t.toString())
                }
    ...
    private fun setIsLoading() {
        supplierObservable.postValue(Resource.loading(null))
    }

    private fun setIsSuccessful(parameters: SupplierData) {
        supplierObservable.postValue(Resource.success(parameters))
    }

    private fun setIsError(message: String) {
        supplierObservable.postValue(Resource.error(message, null))
    }
    fun getToken(): String {
