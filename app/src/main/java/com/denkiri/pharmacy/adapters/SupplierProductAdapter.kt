...
): RecyclerView.Adapter<SupplierProductViewHolder>()  {
    var filterList = ArrayList<Product>()
    init {
        filterList= modelList!!
    }
    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): SupplierProductViewHolder {
        var itemView: View? =null
        itemView = LayoutInflater.from(parent.context).inflate(R.layout.supplier_product_item, parent, false)
        return SupplierProductViewHolder(type, itemView!!, recyclerListener)
    }
    @SuppressLint("SetTextI18n")
    override fun onBindViewHolder(holder: SupplierProductViewHolder, position: Int) {
